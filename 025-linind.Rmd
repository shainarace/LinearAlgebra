...
```
```{def title="Linear Dependence and Linear Independence", id='linind', num=thmcounter, cid=cid}
...
```
```{ex title='Linearly Dependent Vectors', id='lindep', num=excounter, cid=cid}
...
thmcounter=thmcounter+1
```
```{thm title='Equivalent Conditions for Matrix Invertibility', id='equivcond', num=thmcounter, cid=cid}
...
thmcounter=thmcounter+1
```
```{def title='Vector Span', id='span', num=thmcounter, cid=cid}
...
thmcounter=thmcounter+1
```  
```{def title='Subspace', id='subspace', num=thmcounter, cid=cid}
...
```{ex title='Span', id='ex-span',num=excounter, cid=cid}
