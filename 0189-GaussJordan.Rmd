...
3. Replace one equation by a combination of itself plus a multiple of another equation.
...
x_1+2x_2 &=& 11\\
x_1+x_2 &=& 6\end{eqnarray}\end{cases}
\quad \Leftrightarrow \quad \begin{cases}\begin{eqnarray}
x_1+x_2 &=& 6\\
x_1+2x_2 &=& 11\end{eqnarray}\end{cases}$$

2. Multiplying both sides of one equation by a constant.

$$\begin{cases}\begin{eqnarray}
x_1+2x_2 &=& 11\\
x_1+x_2 &=& 6\end{eqnarray}\end{cases} \quad \Leftrightarrow \quad \begin{cases}\begin{eqnarray}
x_1+2x_2 &=& 11\\
2x_1+2x_2 &=& 12\end{eqnarray}\end{cases}$$
...
Using these 3 row operations, we can transform any system of equations into one that is _triangular_. A **triangular system** is one that can be solved by back substitution (backsolved). For example,
...
```
```{ex title='Transforming a System to a Triangular System via 3 Operations',id='ex-rowopeq', num=excounter}
...
```
```{def title='The Augmented Matrix', id='def-augmat',num=thmcounter}
...
thmcounter=thmcounter+1
```
```{def title='Row Operations for Gaussian Elimination', id='def-rowops',num=thmcounter}
...
excounter=excounter+1
```
```{ex title='Row Operations on the Augmented Matrix', id='ex-rowopmat', num=excounter}
...
1.Focusing on the first column, identify the first pivot element. The first pivot element should be located in the first row (if this entry is zero, we must interchange rows so that it is non-zero). 
2. Eliminate (zero-out) all elements below the pivot using the combination row operation.
3. Determine the next pivot and go back to step 2.
...
4. When the entries below all of the pivots are equal to zero, the process stops. The augmented matrix is said to be in _row-echelon form_, which corresponds to a _triangular_ system of equations, suitable to solve using back substitution.
...
excounter=excounter+1
```
```{ex title='Gauss-Jordan elimination', id='ex-gaussjordan', num=excounter}
...
- **Case 1**: The system could have one unique solution (this is the case with the examples thus far).
- **Case 2**: The system could have no solutions (sometimes called _overdetermined_ or _inconsistent_).
- **Case 3**: The system could have infinitely many solutions (sometimes called _underdetermined_).
...
thmcounter=thmcounter+1
```
```{def title='(ref:case1title)', id='case1',num=thmcounter}
...
excounter=excounter+1
```
```{ex title='Identifying an Inconsistent System', id='inconsistent', num=excounter}
$$\begin{cases}\begin{align}
x-y+z=1\\
x-y-z=2\\
x+y-z=3\\
...
x_1+x_3 = 0\\
x_2+x_3= 0\end{align}\end{cases}$$
Clearly, our answer to this problem depends on the variable $x_3$, which is considered _free_ to take on any value. Suppose that $x_3=\alpha$. Then our solution would be:
$$\pm x_1\\x_2\\x_3 \mp = \pm -\alpha \\ -\alpha \\ \alpha \mp = \alpha \pm -1\\-1\\1 \mp$$
...
thmcounter=thmcounter+1
```
```{def title='(ref:case3title)', id='case3', num=thmcounter}
...
excounter=excounter+1
```
```{ex title='Infinitely Many Solutions', id='infsolutions'}
...
excounter=excounter+1
``` 
```{ex title='Finding a Matrix Inverse', id='findinverse', num=excounter}
...
```{exer title='Finding a Matrix Inverse', id='exer-inverse'}
...
excounter=excounter+1
```
```{ex title='Inverse of a Diagonal Matrix', id='ex-diaginverse', num=excounter}
...
thmcounter=thmcounter+1
```
```{def title='Inverse of a Diagonal Matrix', id='def-diaginverse', num=thmcounter}
...
1. Using Gaussian Elimination on the augmented matrices, reduce each system of equations to a triangular form and solve using back-substitution.
  a.$$\begin{cases}
x_1 +2x_2= 3\\
-x_1+x_2=0\end{cases}$$
  b.$$\begin{cases}
...
-2x_1-2x_2 =-6\end{cases}$$
  c. $$\begin{cases}\begin{align}
2x_1-x_2 +x_3= 1\\
-x_1+2x_2+3x_3 = 6\\
x_2+4x_3 =6 \end{align}\end{cases}$$

2. Using Gauss-Jordan Elimination on the augmented matrices, reduce each system of equations from the previous exercise to reduced row-echelon form and give the solution as a vector.

3. Use either Gaussian or Gauss-Jordan Elimination to solve the following systems of equations. Indicate whether the systems have a unique solution, no solution, or infinitely many solutions. If the system has infinitely many solutions, exhibit a general solution in vector form as we did in Section \ref{infinitesol}.
  a. $$\begin{cases}\begin{align}
		2x_1+2x_2+6x_3=4\\
		2x_1+x_2+7x_3=6\\
		-2x_1-6x_2-7x_3=-1\end{align}\end{cases}$$
  b. $$\begin{cases}\begin{align}
		1x_1+2x_2+2x_3=0\\
		2x_1+5x_2+7x_3=0\\
  ...
  d. $\begin{cases}\begin{align}
w-h+l=1\\
w-h-l=2\\
w+h-l=3\\
w+h+l=4\end{align}\end{cases}$
  e. $\begin{cases}\begin{align}
w-h+l=1\\
w-h-l=2\\
w+h-l=3\\
w+h+l=2\end{align}\end{cases}$
\item[f.] $\begin{cases}\begin{align}
...
\item Use Gauss-Jordan Elimination to find the inverse of the following matrices, if possible.
\begin{itemize}
\item[a.] $\A=\pm 2&3\\2&2\mp$
\item[b.] $\B=\pm 1&2\\2&4\mp$
\item[c.] $\C=\pm 1&2&3\\4&5&6\\7&8&9\mp$
\item[d.] $\D=\pm 4&0&0\\0&-4&0\\0&0&2 \mp$
\end{itemize}

\item What is the inverse of a diagonal matrix, $\bo{D}=diag\{\sigma_{1},\sigma_{2},
\dots,\sigma_{n}\}$?

\item Suppose you have a matrix of data, $\A_{n\times p}$, containing $n$ observations on $p$ variables. Suppose the standard deviations of these variables are contained in a diagonal matrix
