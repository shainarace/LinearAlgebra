...
excounter=excounter+1
```
```{ex title='Linear Combinations of Variables',id='ex-lincombvar'}
...
excounter=excounter+1
```
```{ex title='Systems of Equations as Linear Combinations', id='ex-lincom'}
...
Let $$\A = \pm 1 &3 & 3\\2& 4& 6\\4& 8&6 \mp \quad \bo{p}= \pm 10\\15\\20 \mp \quad \bo{c} = \pm 0.50\\0.25\\0.10 \mp.$$
  ...
  __a.__ Determine the matrix-vector product that produces a vector, $\y$ which gives the total cost of creating       each size arrangement (small, medium, and large).
  __b.__ Suppose that an order came in for 2 small arrangements and 2 large arrangements. Let $$\bo{v} = \pm 2\\0\\2 \mp$$
...
__2.__ Write the following system of equations as a matrix-vector product $\A\x=\b$:
\begin{eqnarray}
2x_2 +3x_3&=& 8,
2x_1+3x_2+1x_3 &=& 5,
x_1-x_2-2x_3 &=&-5
...
__3.__ A model is being developed to predict a student's SAT score based upon some numeric attributes. The data being used for this model is provided below:
...
__4.__ Suppose a company collected daily data regarding the sales and revenue of particular products for which prices fluctuate daily:
...
<table>
  ...
  __a.__  Show how you could use matrix addition to compute the total weekly sales and revenue of each product.
  __b.__  Now suppose you find out that both the sales and the revenue numbers in the table above were listed in hundreds (i.e. that 100 widgets were sold on Monday, bringing in \$19,500 in revenue). Using your answer from part a. show how you would use scalar multiplication to represent the exact weekly numbers for revenue and units sold.
...
__6.__ For a general matrix $\A_{m\times n}$ describe what the following products will provide.  Also give the size of the result (i.e. "$n\times 1$ vector" or "scalar").
