...
```{r id='studentgraph', fig.align='center', fig.cap = 'The Coordinate Plane', echo=F, out.width="40%"}
knitr::include_graphics("figs/coordplane.jpg")
```



When we think of our data points this way, we are considering them as linear combinations of  elementary __basis vectors__
...
```
```{def title='Elementary Basis', id = 'elembasis', num=thmcounter, cid=cid}
...
```{r id='coordplaneB2', fig.align='center', fig.cap = '(ref:coordcap)', echo=F, out.width="40%"}
...
Using our new basis vectors, our alternative view of the data is that in Figure \@ref(coordplaneB2rotate).

(ref:coordB2cap) Points plotted in the new basis, $\mathcal{B}$

```{r id='coordplaneB2rotate', fig.align='center', fig.cap = '(ref:coordB2cap)', echo=F, out.width="50%"}
...
In general, we can change bases using the procedure outlined in Theorem \@ref(thm:changebase).

```{r, echo=F} 
thmcounter=thmcounter+1
```
```{thm title='Changing Bases', id = 'changebase', num=thmcounter, cid=cid}
...
```{thm title='Basis', id = 'basis', num=thmcounter, cid=cid}
...
The preceding discussion dealt entirely with bases for $\Re^n$ (our example was for points in $\Re^2$). However, we will need to consider bases for _subspaces_ of $\Re^n$. Recall that the span of two linearly independent vectors in $\Re^3$ is a plane. This plane is a 2-dimensional subspace of $\Re^3$. Its dimension is 2 because 2 basis vectors are required to represent this space. However, not all points from $\Re^3$ can be written in this basis - only those points which exist on the plane. In the next chapter, we will discuss how to proceed in a situation where the point we'd like to represent does not actually belong to the subspace we are interested in. This is the foundation for Least Squares.
...
<li> Consider $\a_1=(1,1)$ and $\a_2=(0,1)$ as coordinates for points in the elementary basis. Write the coordinates of $\a_1$ and $\a_2$ in the orthonormal basis found in exercise 1. Draw a picture which reflects the old and new basis vectors. 
