...
```
```{thm title='Diagonalization of Symmetric Matrices', id='eigsym', num=thmcounter, cid=cid}
...
2. Furthermore, these eigenvectors can be chosen to be _orthonormal_ so that if $\V=[\v_1|\dots|\v_n]$ then
$$\V^{T}\V=\bo{I}$$
...
<center>
<img src=figs/pcpoints.pdf width=50%/>
</center>
...
<center>
<img src=figs/pcpointscenter.pdf eidth=50%/>
</center>
...
<center>
<img src=figs/pcpointspc.pdf width=50% />
</center> 
...
excounter=excounter+1
``` 
```{ex title='The Principal Component Basis', id='pcabasis', num=excounter, cid=cid} 
...
You might be tempted to conclude from Figure \@ref(pcvsreg) that the first principal component and the regression line "ought to be similar." This is a terrible conclusion if you consider a large multivariate dataset and the various regression lines that would predict each variable in that dataset. In PCA, there is no target variable and thus no single regression line that we'd be comparing to. 
...
The covariance matrix is the default for \textsf{R}. The correlation matrix is the default in SAS. The covariance matrix method is invoked by the option:
...
$$\begin{bmatrix}Sepal.Length\\Sepal.Width\\Petal.Length\\Petal.Width\end{bmatrix} =\mathbf{V}\begin{bmatrix} comp_1\\comp_2\\comp_3\\comp_4\end{bmatrix}$$
...
$$Sepal.Length = 0.361 comp_1 - 0.657 comp_2 - 0.582 comp_3 + 0.315 comp_4$$
...
plot(irispca2$scores[,1],irispca2$scores[,2],
     main="Data Projected on First 2  Principal Components",
     xlab="First Principal Component",
     ylab="Second Principal Component",
     col=c("red","green3","blue")[iris$Species])
