...
```
```{thm title='Diagonalization of Symmetric Matrices', id='eigsym', num=thmcounter}
...
```
```{ex title='Eigenvectors of the Covariance Matrix', id='excoveigs'}
...
excounter=excounter+1
``` 
```{ex title='The Principal Component Basis', id='pcabasis', num=excounter} 
...
## Geometrical comparison with Least Squares
 ...
 \begin{figure}[h!]
 \begin{center}
 \includegraphics[scale=.5]{figs/pcvsreg}
 \caption{Principal Components vs. Regression Lines}
 \label{pcvsreg}
 \end{center}
 \end{figure}
...
### Principal Components, Loadings, and Variance Explained {PCAbyeigen}
